...
Why separate: planning inside an implementation prompt drifts into coding before the graph is stable.
...
Write the task graph into `DAG.md` (replace or extend the classifier DAG: keep qa/critic/promoter as required by easy/hard). Also write `nodes/<your-id>.md` summarizing the plan.
