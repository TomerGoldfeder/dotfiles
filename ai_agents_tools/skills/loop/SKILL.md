...
description: >-
  ...
  (e.g. /loop 5m check CI). Use when a task needs multi-turn wakes — pipeline
  babysitting, waiting for deploys, polling until a condition is met. Required
  companion for pr-babysitting while CI is still running.
---
...
2. Start one background shell loop with output monitoring when the harness supports it.
...
2. **If the next run is gated on an event** (a git ref advancing, a log line matching, a file changing, a CI check completing), arm a background watcher that emits the sentinel only when the event fires, with output monitoring on `^AGENT_LOOP_WAKE_<purpose>`. Arm once; skip on later ticks if it's still running.
